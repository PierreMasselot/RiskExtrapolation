...
All components of the dataset are provided in the *data* subdirectory. The subdirectory also contains the script `00_DownloadData.R` that shows how the data were obtained and pre-processed. The only exception is *data/tmean.csv.gz*, which was extracted using Python from ERA5Land. :warning: This script should not be executed, AS it is here mainly to document the data gathering process.
...
2.  Scripts *21_LocationAgeSpecific.R* to *28_Uncertainty.R* implement each corresponding package (sub-section) in section 3 of the paper. Each one relies on previous ones, which means they should be run in order.
